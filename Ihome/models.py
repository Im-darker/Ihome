...
class User(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            "mobile": self.mobile,
            "avatar": constants.QI_NIU_URL + self.avatar_url if self.avatar_url
            else "",
...
class House(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        house_dict ={
            ...
            "img_url": constants.QI_NIU_URL + self.user.avatar_url if self.user.avatar_url else "",
            "order_count": self.order_count,
            "address": self.address,
            "user_avatar": constants.QI_NIU_URL + self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_det_dict(self):
        ...
        # img_urls = []
        # for image in self.images:
        #     img_urls.append(constants.QI_NIU_URL + image.url)
        # house_dict["img_urls"] = img_urls
        #
        # # 房屋设施
        # facilities = []
        ...
        # comments = []
        # orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None) \
        #     .order_by(Order.update_time.desc()).limit(constants.QI_NIU_URL)
        # for order in orders:
        #     comment = {
        #         "comment": order.comment,  # 评论的内容
        #         "user_name": order.user.name if order.user.name != order.user.mobile else "匿名用户",  # 发表评论的用户
